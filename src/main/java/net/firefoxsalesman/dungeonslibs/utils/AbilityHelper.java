...
import net.firefoxsalesman.dungeonslibs.mixin.NearestAttackableTargetGoalAccessor;
import net.firefoxsalesman.dungeonslibs.mixin.TargetingConditionsAccessor;
...
import static net.firefoxsalesman.dungeonslibs.utils.PetHelper.isPetOrColleagueRelation;
...
public class AbilityHelper {
	...
	public static boolean isFacingEntity(Entity looker, Entity target, Vec3 look, int angle) {
		...
		double dotsq = ((relativePosVec.dot(look) * Math.abs(relativePosVec.dot(look)))
				/ (relativePosVec.lengthSqr() * look.lengthSqr()));
	...
	private static boolean isBothPlayerAndNoPvP(LivingEntity attacker, LivingEntity nearbyEntity) {
		...
		// !DungeonsLibrariesConfig.ENABLE_AREA_OF_EFFECT_ON_OTHER_PLAYERS.get();
		return attacker instanceof Player && nearbyEntity instanceof Player;
	}
	...
	public static boolean isAlly(LivingEntity origin, LivingEntity target) {
		...
		if (originOwner != null || targetOwner != null) {
			return isAlly(originOwner != null ? originOwner : origin,
					targetOwner != null ? targetOwner : target);
		}
		return isPetOrColleagueRelation(origin, target)
				...
				|| (isDefaultEnemy(origin) && isDefaultEnemy(target)
						&& !matchesAITargets(origin, target));
	}
	...
	private static boolean matchesAITargets(LivingEntity origin, LivingEntity target) {
		if (!(origin instanceof Mob originMob))
			return true;
	...
	private static Boolean checkTargetGoals(Mob originMob, LivingEntity target) {
		return originMob.targetSelector.getAvailableGoals().stream()
				...
				.map(goal -> matchesPredicate(target,
						((NearestAttackableTargetGoalAccessor<?>) goal).getTargetType(),
						((NearestAttackableTargetGoalAccessor<?>) goal).getTargetConditions()))
				.reduce(false, (o, o2) -> o || o2);
	}

	private static boolean matchesPredicate(LivingEntity target, Class<?> targetType,
		...
		return (targetType == null || targetType.isInstance(target))
				&& (selector == null || selector.test(target));
	}
	...
	private static boolean isEntityBlacklisted(LivingEntity origin, LivingEntity target) {
		if (target.getType().equals(ARMOR_STAND))
			return true;
	...
	public static boolean canApplyToSecondEnemy(LivingEntity attacker, LivingEntity target,
			LivingEntity nearbyEntity) {
